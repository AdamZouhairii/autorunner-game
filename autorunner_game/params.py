...
import random
import os
from human_race.player import Player
from human_race.obstacle import Obstacle, BlueObstacle
from human_race.bonus import Coin, BriseBrick
from human_race.player import Background
from mario_mode.gamelib import mario_main
from human_race.sound import SoundManager
...
class GameParameters:
    def __init__(self):
        ...
        self.menu_background = pygame.image.load(os.path.join(r"autorunner_game\data\menu_background.jpg")).convert()
        pygame_icon = pygame.image.load(r'autorunner_game\data\icon.png')
        pygame.display.set_icon(pygame_icon)
        pygame.display.set_caption("Subway hagar")
        self.background = Background(r'autorunner_game\data\moving_background.png', self.width, self.height)
        self.obstacle_image = pygame.image.load(r"autorunner_game\data\buche.jpg")
        self.player_images = [pygame.image.load(r'autorunner_game\data\Sanji.png'),
                              pygame.image.load(r'autorunner_game\data\zoro.png'),
                              pygame.image.load(r'autorunner_game\data\Mario.png'),
                              pygame.image.load(r'autorunner_game\data\Luffy.png'),
                              pygame.image.load(r'autorunner_game\data\dragon.png'),
                              pygame.image.load(r'autorunner_game\data\crane.png'),
                              pygame.image.load(r'autorunner_game\data\crane_skin.png'),
                              pygame.image.load(r'autorunner_game\data\chien.png'),
                              pygame.image.load(r'autorunner_game\data\ange.png')]
        #couleur
    ...
    def increase_screen_size(self):
        ...
        self.width, self.height = self.screen_sizes[self.current_screen_size_index]
        self.screen = pygame.display.set_mode((self.width, self.height))
        self.background.resize(self.width, self.height)  # Redimensionner l'image de fond
        ...
        while True:
            ...
            for obstacle in obstacles_collected:
                if isinstance(obstacle, BlueObstacle):
                    if self.blue_coins_owned > 0:
                        obstacle.reset()
                        self.sound_manager.play_blue_obstacle_break_sound()
            ...
            for coin in coins_collected:
                if isinstance(coin, Coin):
                    self.sound_manager.play_yellow_coin_sound()
                    self.yellow_coins_collected += 1
                    self.score += 10
                elif isinstance(coin, BriseBrick):
                    self.sound_manager.play_blue_coin_sound()
            ...
            # Dessin
            self.screen.blit(self.background.image_original, self.background.rect)
    ...
    def menu_select_game(self):
        ...
        while True:
            ...
            title_text = font.render("Select Game", True, (255, 255, 255))
            self.screen.blit(title_text, (self.width/2 - title_text.get_width()/2, 50))

            play_text = font.render("Human race", True, (255, 255, 255))
            play_rect = play_text.get_rect(center=(self.width/2, 150))
            self.screen.blit(play_text, play_rect)

            settings_text = font.render("mario mode", True, (255, 255, 255))
            settings_rect = settings_text.get_rect(center=(self.width/2, 200))
            self.screen.blit(settings_text, settings_rect)

            quit_text = font.render("retour", True, (255, 0, 0))
    ...
    def draw(self):
        self.screen.blit(pygame.transform.scale(self.background.image, (self.width, self.height)), (0, 0))
