...
from mario_mode.gamelib.sprites import *
from mario_mode.gamelib.level import Level
...
class Camera(object):
    def __init__(self, player, width):
        ...
        Attributes:
            player: The player object to follow.
            rect: The rectangle representing the camera view.
            world: The rectangle representing the game world.
        """
        self.player = player
    ...
    def draw_sprites(self, surf, sprites):
        """
        Draws the sprites that are within the camera view.

        Args:
            surf: The surface to draw the sprites on.
            sprites: The list of sprites to draw.
        """
        for s in sprites:
...
class Game(object):
    ...
    Args:
        ...
        continuing (bool, optional): Whether the game is continuing from a saved level. Defaults to False.
    """
        ...
        while self.running:
            ...
            for b in self.bosses:
                ...
                if b.die_time <= 0 and b.dead and not self.explosions:
                    pygamesprite.Sprite.kill(b)   # Just a bug no problem
                    self.next_level()
                if b.die_time > 0:
                    for s in self.shots:
                        s.kill()
